...
try {
    ...
    $query = $connection->prepare("SELECT users.user_id, users.firstname, users.lastname, users.birthday, users.gender, users.user, users.user_profile_picture, CONCAT(address.city, ', ', address.province) AS address FROM users INNER JOIN address ON users.user_id = address.user_id LIMIT :offset, :recordsPerPage");
